  ...
  const performSearch = async (keyword) => {
    ...
    try {
      const [usersResponse, postsResponse] = await Promise.all([
        fetch(
          searchQueryURL +
            `${encodeURIComponent(keyword)}&pageNumber=1&pageSize=12`
        ),
        fetch(
          searchKeywordURL +
            `${encodeURIComponent(keyword)}&pageNumber=1&pageSize=12`
    ...
    <div className="search-container-search" ref={searchRef}>
      ...
      {showResults && (
        <div ref={resultsRef} className="search-results-search">
          ...
          {searchResults.error && (
            <div className="error-container-search">{searchResults.error}</div>
          )}
                      ...
                      <div className="post-content-search">
                        ...
                        <div className="post-details-search">
                          <div className="post-header-search">
                            <span className="author-username-search">
                              @{post.authorUsername}
                            </span>
                            <span className="post-date-search">
                              {formatDate(post.createdAt)}
                            </span>
                            <span className="media-type-search">
